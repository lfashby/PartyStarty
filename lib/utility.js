    ...
    .exec((err, events) => {
      if(err) {
        ...
        events.forEach((event) => {
          ...
          if (!(eventYear > todaysYear) && (eventYear < todaysYear || (eventMonth < todaysMonth) || eventDay < todaysDay)) {
            Invite.remove({ eventId: event._id })
            Message.remove({ eventId: event._id })
            Movie.remove({ eventId: event._id })
